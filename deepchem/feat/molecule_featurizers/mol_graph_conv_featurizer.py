...
from deepchem.feat.molecule_featurizers.molcurvature import get_geom
...
def _construct_atom_feature(atom: RDKitAtom, 
  ...
  atom_feat = np.concatenate([
      #atom_type,
      #formal_charge, hybridization, acceptor_donor, aromatic, 
      degree
      #total_num_Hs
  ])
  ...
  if use_geom:
    ...
    atom_feat = np.concatenate([atom_feat,np.array([curvature,ad]),np.array(normal)])
  else:
    normal = [0,0,0] 
...
class MolGraphConvFeaturizer(MolecularFeaturizer):
  ...
  def _featurize(self, datapoint: RDKitMol, **kwargs) -> GraphData:
    ...
    if use_geom:
      ...
      print("Without geometry features.")
      mol_geom = {}
